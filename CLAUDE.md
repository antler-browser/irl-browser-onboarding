...
│   ├── demos/
│   │   ├── BasicDemo.tsx       # Hook testing (useIrlOnboarding, useProfile)
...
- ✅ Both hooks (useIrlOnboarding, useProfile)
...
- `api.ts`: Mock `IRLBrowser` API implementation that returns signed JWTs

**Utils Layer** (`src/utils/`):
- `deviceDetection.ts`: Platform detection (iOS/Android/Web), IRL Browser detection
...
- `components/`: Main `IrlOnboarding` wrapper, download prompts, 3-step account creation flow
- `hooks/`:
  - `useIrlOnboarding()`: Returns `{shouldShowOnboarding, profile, isLoading}` - simplified API for status detection
...
- Private keys stored in browser LocalStorage (`irl-onboarding:privateKey`)
- JWTs signed with EdDSA algorithm, 2-minute expiration
- Mock API (`api.ts`) mimics native IRL Browser Specification API
- **Important**: All cryptography happens client-side; no server-side key storage

### Key Integration Points

When the user opens a mini-app:
1. `useIrlOnboarding()` detects if native IRL Browser exists via `window.irlBrowser` and returns:
...
3. `createProfile()` in `profile.ts` generates DID, saves to LocalStorage, injects mock API at `window.irlBrowser`
...
- **Spec Compliance**: Follows IRL Browser Specification for API compatibility
